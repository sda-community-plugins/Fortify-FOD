// --------------------------------------------------------------------------------
// Uploads a Zip file containing source files and dependencies to Fortify On Demand
// and initiates a static scan.
// --------------------------------------------------------------------------------
...
try {
    f = new File(zipFile)
    if (!f.exists()) {
        throw new StepFailedException("Zip file \"${zipFile}\" does not exist!")
    }
