// --------------------------------------------------------------------------------
// Retrieve the security rating of a release and fail if a minimum value has not
// been reached.
// --------------------------------------------------------------------------------
...
Integer minRating = props.notNullInt("minRating")
...
println "Minimum Rating: ${minRating}"
...
try {
    ...
    rating = release.getRating()
    println "Application: ${release.getApplicationName()} - ${release.getReleaseName()} security rating: ${rating}"
    if (rating >= minRating) {
        println "Minimum rating achieved."
    }  else {
        throw new StepFailedException("Minimum security rating not achieved")
    }
